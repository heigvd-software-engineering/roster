...
/**
 * The GitHub App install Setup URL callback. Two jobs with two threat models,
 * hence two paths.
 *
 * REPAIR (an existing class). A reinstall mints a new installation id, and
 * GitHub fires the Setup URL in whatever browser performed it, possibly with
 * no roster cookie (the org-settings page, or a second org owner who has never
 * signed in here). `installationAccount` runs on the App's own JWT, so GitHub,
 * not the caller, names the org that owns this installation. An attacker
 * passing an arbitrary `installation_id` cannot choose where the write lands:
 * GitHub resolves it to that installation's true org, and an App has exactly
 * one installation per org, so the worst achievable write is the correct value
 * or a no-op. The repair therefore needs no session, no token, and no
 * ownership check, and it writes only the installation pointer and the org
 * identity cache: never `status` (a session-less call must not resurrect a
 * deactivated class), never `joinToken` (the cohort's link), never provenance.
 *
 * CREATE (a new class). Now provenance matters. The cookie alone does not
 * prove this user installed the App, and installation ids are small enumerable
 * ints, so any signed-in user could otherwise claim any org's installation as
 * their own class. We verify the caller holds this installation (`GET
 ...
 * `class_members` display cache so a fresh class card names its teachers and
 * students at once, instead of showing 0/0 until a reconcile. Same mapping as
 * the roster reconciler (Owner → teacher, member → active, invitee → pending),
 * add and update only; removals stay reconcile's job, behind a teacher's
 * consent. Display cache, so it authorizes nothing, and a failure here must
 * never break the connect: reconcile can backfill.
 */
async function seedRoster(
  ...
  try {
    ...
    // `orgPeople` reports pending people by invitation id, because the
    // invitations API never returns the invitee's user id, so those seed with
    // no `githubId`. Everyone else is a real user id.
    const rows = [
...
export const githubSetupCallback = factory.createHandlers(async (c) => {
  ...
  // columns. Nothing else writes them: the hub no longer does, and the
  // `identity` reconciler runs only when a teacher opens the page. Without
  // this a freshly connected class has no name until someone reconciles it.
  //
  // It also covers the one drift reconciliation cannot reach: while the App is
  // uninstalled the class vanishes from the hub (no installation token, so no
  // way to authorize the caller), and reconnecting here is the only way back
  // in. If the org was renamed meanwhile, this is where we notice.
  ...
  if (existing) {
    await db
      .update(classes)
      .set({
        // Pointer and identity only. Never `status` (a session-less call must
        // not resurrect a deactivated class), never `joinToken`, never
        // provenance.
        installationId,
        login: org.login,
        name: org.name,
  ...
  // class is born, one condition for everyone, super admins included. The
  // repair path above never reaches this line, so existing classes are
  // untouched.
