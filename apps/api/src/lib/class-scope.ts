 ...
 * `resolveClassAsTeacher`, use deliberately different mechanisms; see each
 * below.
 *
 ...
 * class_members display cache.
 */

type Db = ReturnType<typeof getDb>;

/** The class org's GitHub Team API, pre-bound to this class's installation and
...
type ClassTeam = {
  ...
  /** Mirror one team's live roster into `group_members`. Null means the team
   *  is gone. */
  syncMembers: (
...
export type ClassScope = {
   ...
   *  side as `org`/`login` and nothing said which was whose. Self join and
   *  leave act on this one. */
  callerLogin: string;
  /** Live org Owner, i.e. a teacher of this class. Named for the app's role
   *  rather than GitHub's `admin`, since every caller reads it as "may they
   *  manage this class". */
  isTeacher: boolean;
  /** Live org membership state. "pending" reaches only the callers that opted
   *  in via `allowPending`. */
 ...
 * are a teacher, their live membership state, and the class's Team API
 * pre-bound to this installation.
 *
 ...
 * than 403 so the answer never reveals a class to an outsider.
 *
...
export async function resolveClassAsMember(
  ...
  opts?: {
    /** Also resolve a pending invitee, for read-only surfaces that must tell
     *  "accept your invitation first" apart from "not your class". The default
     *  stays active-only: a pending member can't act. */
  ...
  /** The caller's current login, straight from GitHub: the fallback when the
   *  cache can't answer (no row, or a renamed login). */
  ...
  try {
    // Both names come from caches, so zero calls on the hot path: the caller's
    // login from the class's member cache, the org's login from the class row's
    // identity cache (kept true by setup.ts and the identity reconciler).
    // Caches only propose names; the live orgMembership call below authorizes.
    ...
    // renamed, or both. Re-derive whatever came from a cache and retry once.
    // A real non-member still ends at null.
    ...
    // Pending invitees can't act yet, so active members only unless the caller
    // asked to see the pending state.
    ...
    return {
      ...
      org,
      callerLogin: login,
      // An Owner is a live active admin; a pending Owner invite is not one.
      isTeacher: state === "active" && membership?.role === "admin",
      membershipState: state,
      team: classTeam(db, c.env, cls.installationId, org),
    };
  } catch (err) {
    // GitHub being unreachable is not "this class doesn't exist", so let the
 ...
 * stored account id is an Owner. It never touches the caller's OAuth token, so
...
export async function resolveClassAsTeacher(
  ...
  try {
    // Org login from the class row's identity cache: zero calls, live only when
    // unfilled. A stale cached login (org renamed) makes the members listing
    // throw 404, and that alone warrants re-deriving it live and retrying once.
    // `false` is a real non-admin, so no retry.
    let org = cls.login ?? (await orgLogin(c.env, cls.installationId));
    let admin: boolean;
    try {
