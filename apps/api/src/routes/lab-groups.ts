...
// A lab's groups (per-lab model): groups are born in a lab, so create lives
// here (POST /groups, optionally copying a roster forward). Group and lab are
// both scope-checked against the class. `accept` is the one-click individual
// path (solo group + repo), `repo` the explicit completion step for group labs
// (min enforced there), and `repos` the teacher's batch of every missing one.
