...
// Group membership and lifecycle by group id (per-assignment model: a group
// belongs to one assignment, so these stay class-scoped by the globally unique
// group id). Create is assignment-scoped (routes/assignment-groups.ts).
// `membership` is the caller's own (self join/leave), `members/:login` is a
// teacher acting on someone else, and `repo` is the teacher's escape hatch for
// a repo deleted on GitHub.
