...
export function MainSwitchIdentity() {
  ...
  // Owned here, not by the group in the menu: picking "Revoke access" closes
  // the menu and unmounts its content, and the confirm dialog must outlive
  // that (ConfirmDialog's controlled mode; the group card's kebab precedent).
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="min-w-64">
          ...
          <DropdownMenuSeparator />
          {/* Standing grants: the list behind "access can be withdrawn at any
            time". Same shape as the GitHub group above on purpose. */}
          <ConnectedAssistants onRevoke={setRevokeTarget} />
          <DropdownMenuSeparator />
          <DropdownMenuRadioGroup
            value={theme}
