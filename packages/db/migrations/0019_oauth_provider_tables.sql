-- Generated, not written: registering a Better Auth plugin IS a schema change.
-- Each plugin declares the tables it needs; the CLI builds the auth config,
-- walks the registered plugins, and collects those declarations. The input is
-- the plugin list in lib/auth/config.ts — nothing here is inferred from the
-- database, and regenerating with a plugin removed drops its tables.
--
-- PROVENANCE — reproduce with, from apps/api and packages/db respectively:
--   pnpm --filter @roster/api auth:schema     (auth@1.7.2 generate
--       --adapter drizzle --dialect sqlite → packages/db/src/auth-schema.ts)
--   pnpm --filter @roster/db db:generate      (drizzle-kit 0.31.10 diffs that
--       schema against meta/0018_snapshot.json and writes this file)
--   Produced by better-auth@1.7.2 + @better-auth/mcp@1.7.2, 2026-08-28.
--   `auth generate` needs --adapter/--dialect and the in-memory adapter seam in
--   better-auth.config.ts: the Drizzle adapter validates every model while it
--   constructs, so the tables about to be written make it refuse to start.
--
-- DOCUMENTATION
...
--   Design decisions      brainstorm board, decisions #2, #6, #9, #12, #13
--
-- WHAT CAME FROM WHERE. Two plugins were added for the MCP lane; the 1.7
-- upgrade itself added one column and that was 0018. Table names and columns
-- below are the plugins' own (read back from the generated schema); the
-- "used/inert" notes are ours, and follow from the options in config.ts:
--
--   jwt()  → jwks — the keys that sign the tokens roster issues
--   mcp()  → the oauth_* tables, because mcp() IS the OAuth provider:
--            oauth_client            registered assistants (dynamic registration writes here)
--            oauth_consent           a teacher's grant, re-read on every tool call
--            oauth_access_token      issued tokens, so revocation does not wait for expiry
--            oauth_resource,         RFC 8707 audience binding: this token is for /mcp
--            oauth_client_resource   (the `resource` option in config.ts)
--            oauth_refresh_token     inert — grantTypes is authorization_code only
--            oauth_client_assertion  inert — our clients are public and use PKCE
--
-- Additive throughout: no table is rebuilt and none is dropped.
