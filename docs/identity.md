...
The caches (`classes.login`, `class_members.login`, `group_members`) only propose names to ask about. A membership miss re-derives whatever came from a cache and retries once before believing it. No cache ever authorizes: `ClassTeam.roster` is documented as the live read to use wherever the answer gates an irreversible write, with the cache reserved for display. See [data model](./data-model.md) for the tables and [classes and labs](./classes-and-labs.md) for what the roles unlock.
