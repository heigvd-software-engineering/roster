...
| F7 | Groups (create/join/leave/remove/delete) | `groups` | [x] **DONE + live 🔴 walk PASSED** (2026-07-06) — see "Feature 7 — Groups" section below; gate closed 104 api / 66 www. |
| F8 | Accept a lab → student lab repo (solo = team of one) | `labs` template columns + `student_lab_repos` repo columns (0009) | [x] **DONE + live 🔴 walk PASSED** (2026-07-07) — see "Feature 8 — Work repos" section below; gate 104 api / 70 www. Org-owned work repos (student-owned-repos idea stays parked). |
| F9 | Student home (browse classes/labs/standing) | none | [x] **DONE as scoped** — class-list slice 2026-07-06 (`class_members` cache + org cache (0005), join write points, `enrolled` on GET /api/classes, EnrolledClassCard + role markers); accept flows landed with F7/F8 on the lab pages. **Standing column DROPPED** (user-decided 2026-07-07): the hub stays lean, standing lives on the lab pages; the Progress column was removed outright. |
...
|---|---|---|---|
| 2026-07-07 (PER-LAB GROUPS) | **F7/F8 core reshaped: groups are now PER-LAB** (spec `2026-07-07-per-lab-groups-design.md`) — fixes the cross-lab dead-end (a reusable atomic team couldn't express "different teammates per lab"). A group belongs to ONE lab + owns its GitHub team; `name` (display, reusable per lab) / `slug` (`labSlug-groupSlug`, sent to GitHub) / `ghTeamSlug` (returned). Migration 0010: `groups` gains lab_id+slug, drops class_id, folds in repo columns; `student_lab_repos` dropped (class DERIVED via lab). Wiped + re-formed test groups/teams (no data migration). API: `createGroupInLab` (+copy-forward roster), within-lab-only `alreadyInLabGroup`, repo named by `group.slug`, attach/detach GONE. www: hook exposes `groups` (repo/activity inline), teacher drops the attach menu (drawer detach→delete group), student drops accept-with-existing (Join + New group). **Student COPY-FORWARD** built: `GET …/reusable` (caller's other-lab groups) + a "Start from / Reuse" picker in the New-group dialog. **Bug fixed live:** empty template repo → 422 was mislabeled "name taken"; now distinguished as `template_error` with an actionable message. Test gate: **102 api / 79 www.** | **Teacher bulk copy-forward** + **group-membership cache** (slice 4) — or F10 | UNCOMMITTED (big batch). Remote D1 0005–0010 pending. Orphaned old GitHub teams may linger (delete per org). Caveat: lab-title-based slug collides if two labs share a title. |
| 2026-07-07 (hub paging + polish) | **Hub paged by SEMESTER, all gates passed** (call-efficiency: every visible teaching class costs live GitHub work): `GET /api/classes?from=` windows by class creation date BEFORE GitHub calls + `hasOlder` (pure-DB probes); calendar generalized to ordered terms incl. **Summer school** (Spring Feb–Jun · Summer Jul–Aug · Autumn Sep–Jan, gap-free by construction, `SEMESTER_CONFIG` one-line edits); hub loads current semester only + full-width dashed "Load {semester}" (disabled at the bottom, hidden only on an empty hub); empty semesters auto-skip (capped 6/gesture; **stale-data bug found live and fixed** — the skip must wait out in-flight fetches or keepPreviousData chains it to the cap). `useApi` cache keys now carry the QUERY (windows collided on pathname). Also: attach menu matched to the approved mockup (Pill vocabulary, member avatars, blocked-with-reason); Progress column DROPPED (user-decided — standing lives on lab pages); "+ Connect an organization" ghost card REMOVED (header dialog remains); labs-table header gutter aligned with action rows. Test gate: **109 api / 80 www**. | **F10 teacher dashboard** or user's call | UNCOMMITTED (batch pending say-so). Remote D1 migrations 0005–0009 pending. Node 22.14 warning. Two App-permission approvals pending. Local D1: Test-TWeb-2027 backdated to 2026-03-01 for paging tests (was 2026-07-02). |
| 2026-07-07 (activity slice) | **Roster activity + call-efficiency, all gates passed.** API: installation-token cache per isolate in `github/clients.ts` (was: one token mint per operation; only the token STRING is cached — Workers forbids cross-request I/O sharing); `orgRepoActivity` = ONE `GET /orgs/{org}/repos` listing covers every work repo's `pushed_at`/`created_at`; `listLabGroups` pairings carry `pushedAt`/`repoCreatedAt` (listing failure degrades chips, never the roster); **batch endpoint `POST …/labs/:labId/repos`** (teacher-only, sequential on purpose vs GitHub abuse limits, `{created, skipped[reason]}`, short-circuits on `app_permissions`) replaces the toolbar's N create+refetch loop. Statuses (shared hook): on_track/on_time (teal) · late (red) · no_pushes (creation-commit-aware, 2-min threshold) · ready = activity unknown. UI: Last-push column ("5h before deadline"), late stat (red>0), Late filter segment; **attach menu enriched** (all unattached groups w/ member avatars; over-max AND already-participating listed disabled w/ reason). Test gate: **107 api / 74 www** (batch + activity + teacher-only covered). | **F9 remainder** (student standing on the hub — reuses statusFor) or F10 | UNCOMMITTED (batch pending say-so). Remote D1 migrations 0005–0009 pending. Node 22.14 warning. TEST-PRG1-2026 + Test-TWeb-2027 App approvals pending. |
| 2026-07-07 (teacher roster) | **Teacher lab page rebuilt as an ASSIGNMENT ROSTER** (GitHub-Classroom-like; mockup approved via artifact, visual gates ✅): stat strip on a card (groups · placed x/y · repos x/y, `lab-stats.tsx`), pool restyled as the amber warning banner (shared w/ student page), toolbar = search + All/Needs-attention/Ready segments + bulk "create N missing repos" + Attach-a-group (stays visible, disabled when none left) + New group (NewGroupDialog gained a `trigger` override); table rows = group/members(AvatarGroup)/repo/status chip + colored spine (`roster.tsx`); management = expandable row drawer (GroupMembers reuse, card-like member rows, add-from-pool, detach DISABLED once the repo exists — mirrors server orphan protection). Shared-altitude moves: `statusFor` + `placedCount` on `use-lab-groups`, `usersByGithubId` in lib/format, `CAPS_LABEL` typography token (also de-duped lab-row). `CloneCommands` extracted/shared → individual accept page now shows the copyable clone snippet (full width). Simplify pass (4-angle) applied; `teacher-lab-groups` decomposed (RosterToolbar/AttachGroupMenu/GroupRow/GroupDrawer, file-local). Test gate: 104 api / **74 www**. | **Activity slice** (last push / late chips + late stat; needs per-repo `pushed_at` + request-scoped GitHub client reuse + batch create-missing-repos endpoint) — or user's call | UNCOMMITTED (batch pending say-so). Remote D1 migrations 0005–0009 still pending. Node 22.14 warning persists. TEST-PRG1-2026 + Test-TWeb-2027 App-permission approvals still pending. |
| 2026-07-07 (F8 polish) | **F8 live-confirmed** (App perms approved per org — verified via App JWT that Test-PRG2-2026 + Test-TWeb-2026 carry administration+contents write; TEST-PRG1-2026 + Test-TWeb-2027 still pending approval). **Student lab page restructured** (visual gate ✅): two explicit states — BROWSE (all participating groups + accept ghosts) vs YOUR GROUP (other groups hidden; group tile 1/3 + **StartLabCard** 2/3); the card owns the repo: "Create repository" once the group reaches min, then repo link + copyable `git clone`/`cd` snippet; ring turns role-enrolled ONLY once the repo exists (waiting = neutral; Card outline is a RING not a border — gotcha). Tile footer (old create button/link) removed from the student view. Test gate: 104 api / **73 www**. **Batch committed.** | **F9 remainder** (student standing/accept views) or user's call | Remote D1 migrations still pending (0005–0009, needs wrangler auth). Node 22.14 warning persists. Deferred carry-overs unchanged (solo-name keyed column, GitHub client reuse, server-side size verdicts). |
| 2026-07-06/07 (F8) | **Feature 8 Work repos COMPLETE, all gates passed** (see the F8 table): labs carry an optional template repo (TemplatePicker in the lab dialog + org templates endpoint); repo per group↔lab pairing (generate-from-template or empty, `lib/github/repo.ts`, slugify w/ Unicode combining-char fix); individual accept auto-creates (solo team-of-one), group labs = explicit create action both roles; repo links on tiles; orphan protection on detach + group delete; migration 0009 local. Live walk found the 500 → GitHub App lacked **Repository: Administration** (403); user updated App perms + approved on the installation → creation works live (2026-07-07 session start). Hardening: 403 → 409 `app_permissions` actionable error (3 call sites); `GITHUB_APP_SETUP.md` documents Repository Administration + Contents R&W + approval flow. Gate: 104 api / 70 www. | **F9 remainder** (student standing/accept views) or user's call | Whole F8 increment UNCOMMITTED (user-triggered batch pending). Remote D1 migrations still pending (0005–0009, needs wrangler auth). Node 22.14 warning persists. Deferred carry-overs: solo-name keyed column, request-scoped GitHub client reuse, server-side size verdicts. |
...
# Feature 8 — Work repos (built 2026-07-06, live-passed 2026-07-07)

**Outcome:** every group↔lab pairing gets an **org-owned work repository** —
generated from the lab's optional **template repo** or created empty — with the
group's team granted access. Individual labs create the repo on one-click
accept (solo team-of-one); group labs get an explicit "create repo" action
(both roles). Repos are protected from orphaning: detach and group-delete
refuse while a work repo exists.

| Task | Auto | Human | Done |
|---|---|---|---|
| 1. Schema: `labs` template-repo columns + `student_lab_repos` repo columns; migration 0009 (applied local) | [x] | [x] 🟢 | [x] |
| 2. `lib/github/repo.ts` — repo ops (create in org / generate from template, slugified names incl. Unicode combining-char fix) | [x] | [x] 🟢 | [x] |
| 3. API: `createPairRepo` orchestration; lab-groups list carries repo info; explicit `POST …/groups/:id/repo`; individual accept auto-creates; orphan protection on detach AND group delete; org templates list endpoint; lab create/update accept a template repo | [x] | [x] 🟢 | [x] |
| 4. UI: repo links on group tiles; create-repo actions in teacher + student views; individual accept shows the repo; lab dialog gains the TemplatePicker | [x] | [x] 👁 | [x] |
| 5. Live walk 🔴: first repo creation 500 → traced to the GitHub App missing **Repository: Administration** permission (403 `Resource not accessible by integration`); App perms updated + installation approved by user → **repo creation works live** (2026-07-07). Hardening kept: GitHub 403 → 409 `app_permissions` with an actionable message at all three call sites; `GITHUB_APP_SETUP.md` now documents Repository Administration + Contents (Read & write) and the installation-approval flow | [x] | [x] 🔴 | [x] |

**F8 gate closed 2026-07-07:** biome ✅ typecheck 3/3 ✅ tests 104 api / 70 www ✅.
