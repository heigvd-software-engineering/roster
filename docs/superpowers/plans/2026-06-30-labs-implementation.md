...
>
> _**Next: Feature 3 — Teacher connects a class** (still only OUTLINED below — expand into tasks first; 🔴 needs GitHub App setup + live creds). Deferred: silent `getAccessToken` token refresh (chose onboarding-re-link resilience instead). See ledger `.superpowers/sdd/progress.md`._
