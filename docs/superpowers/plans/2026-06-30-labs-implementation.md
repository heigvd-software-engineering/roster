...
| F5 | View class people (live Owners/Members) | none (read live) | **F5a (teacher access model) [x] DONE + live 🔴 PASSED** (2026-07-02, pulled ahead of F4): teacher = live org `admin`; list = installations ∩ rows + admin filter; writes 404 for non-admins; co-owner + demote both live-verified. See `specs/2026-07-02-f5a-teacher-access-design.md`. F5b people chips [x] **DONE** (2026-07-03, spec `2026-07-03-f5b-class-people-design.md`): live teachers/students/pending on /api/classes (orgPeople, one fetch also drives the F5a check), clickable PeopleChip popovers. Health chip still deferred to class detail. UNCOMMITTED. |
