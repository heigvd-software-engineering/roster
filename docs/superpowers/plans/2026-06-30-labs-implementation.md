...
**▶ Active cursor:** _On branch `milestone-4-labs`. **F8 committed** (repos: `49b9a69`…`a32ff4a`). **Teacher ASSIGNMENT ROSTER built + gated 2026-07-07** (UNCOMMITTED, commit = user-triggered batch). **Next: the activity slice** (last-push/late indicators; per-repo `pushed_at`, GitHub client reuse, batch repo creation) or F9 remainder._
